...
export const useSnapshotStore = defineStore('snapshot', {
    ...
    actions: {
        ...
        async addSnapshot() {
            ...
            if (snapshotLength >= 2) {
                db.snapshots.update(allKeys[snapshotLength - 2], { index: slidesStore.slideIndex });
            }
        ...
        async unDo() {
            ...
            const snapshots = await db.snapshots.orderBy('id').toArray();
        ...
        async reDo() {
            if (this.snapshotCursor >= this.snapshotLength - 1)
                return;
            const slidesStore = useSlidesStore();
            const mainStore = useMainStore();
            const snapshotCursor = this.snapshotCursor + 1;
