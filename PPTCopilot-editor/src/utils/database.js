...
export const deleteDiscardedDB = async () => {
    ...
    const localStorageDiscardedDBList = localStorageDiscardedDB ? JSON.parse(localStorageDiscardedDB) : [];
    const databaseNames = await Dexie.getDatabaseNames();
    const discardedDBNames = databaseNames.filter(name => {
        if (name.indexOf(databaseNamePrefix) === -1)
            return false;
        const [prefix, id, time] = name.split('_');
        if (prefix !== databaseNamePrefix || !id || !time)
            return true;
        if (localStorageDiscardedDBList.includes(id))
            return true;
        if (now - (+time) >= 1000 * 60 * 60 * 12)
            return true;
        return false;
    });
    for (const name of discardedDBNames)
        Dexie.delete(name);
