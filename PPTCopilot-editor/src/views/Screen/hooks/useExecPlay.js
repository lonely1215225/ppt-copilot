...
export default () => {
    ...
    // 鼠标滚动翻页
    const mousewheelListener = throttle(function (e) {
        if (e.deltaY < 0)
            execPrev();
        else if (e.deltaY > 0)
            execNext();
    ...
    const keydownListener = (e) => {
        ...
        if (key === "ARROWUP" /* KEYS.UP */ || key === "ARROWLEFT" /* KEYS.LEFT */ || key === "PAGEUP" /* KEYS.PAGEUP */)
            execPrev();
        else if (key === "ARROWDOWN" /* KEYS.DOWN */ ||
            key === "ARROWRIGHT" /* KEYS.RIGHT */ ||
            key === " " /* KEYS.SPACE */ ||
    ...
    // 切换幻灯片到指定的页面
    const turnSlideToIndex = (index) => {
        slidesStore.updateSlideIndex(index);
        animationIndex.value = 0;
    };
    const turnSlideToId = (id) => {
...
//# sourceMappingURL=useExecPlay.js.map