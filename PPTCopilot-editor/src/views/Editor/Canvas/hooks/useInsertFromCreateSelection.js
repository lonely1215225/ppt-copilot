...
export default (viewportRef) => {
    ...
    const formatCreateSelectionForLine = (selectionData) => {
        const { start, end } = selectionData;
        if (!viewportRef.value)
            return;
        const viewportRect = viewportRef.value.getBoundingClientRect();
    ...
    // 根据鼠标选区的位置大小插入元素
    const insertElementFromCreateSelection = (selectionData) => {
