...
const getOppositePoint = (direction, points) => {
    const oppositeMap = {
        ["right-bottom" /* OperateResizeHandlers.RIGHT_BOTTOM */]: points.leftTopPoint,
        ["left-bottom" /* OperateResizeHandlers.LEFT_BOTTOM */]: points.rightTopPoint,
        ["left-top" /* OperateResizeHandlers.LEFT_TOP */]: points.rightBottomPoint,
        ["right-top" /* OperateResizeHandlers.RIGHT_TOP */]: points.leftBottomPoint,
        ["top" /* OperateResizeHandlers.TOP */]: points.bottomPoint,
        ["bottom" /* OperateResizeHandlers.BOTTOM */]: points.topPoint,
        ["left" /* OperateResizeHandlers.LEFT */]: points.rightPoint,
        ["right" /* OperateResizeHandlers.RIGHT */]: points.leftPoint,
    };
    return oppositeMap[direction];
};
export default (elementList, alignmentLines, canvasScale) => {
    ...
    // 缩放元素
    const scaleElement = (e, element, command) => {
        ...
        const getSizeWithinRange = (size) => size < minSize ? minSize : size;
        let points;
        let baseLeft = 0;
        let baseTop = 0;
        let horizontalLines = [];
        ...
        // 其中线条和被旋转过的元素不参与吸附对齐
            ...
            for (const el of elementList.value) {
                ...
                if (el.type === 'line')
                    continue;
                if (isActiveGroupElement && el.id === element.id)
                    continue;
                if (!isActiveGroupElement && activeElementIdList.value.includes(el.id))
                    continue;
                const left = el.left;
                const top = el.top;
                ...
                const topLine = { value: top, range: [left, right] };
                const bottomLine = { value: bottom, range: [left, right] };
                const leftLine = { value: left, range: [top, bottom] };
                const rightLine = { value: right, range: [top, bottom] };
                horizontalLines.push(topLine, bottomLine);
                verticalLines.push(leftLine, rightLine);
            }
            // 画布可视区域的四个边界、水平中心、垂直中心
            const edgeTopLine = { value: 0, range: [0, edgeWidth] };
            const edgeBottomLine = { value: edgeHeight, range: [0, edgeWidth] };
            const edgeHorizontalCenterLine = { value: edgeHeight / 2, range: [0, edgeWidth] };
            const edgeLeftLine = { value: 0, range: [0, edgeHeight] };
            const edgeRightLine = { value: edgeWidth, range: [0, edgeHeight] };
            const edgeVerticalCenterLine = { value: edgeWidth / 2, range: [0, edgeHeight] };
        ...
        const handleMousemove = (e) => {
            ...
            if (elRotate) {
                ...
                if (fixedRatio) {
                    if (command === "right-bottom" /* OperateResizeHandlers.RIGHT_BOTTOM */ || command === "left-top" /* OperateResizeHandlers.LEFT_TOP */)
                        revisedY = revisedX / aspectRatio;
                    if (command === "left-bottom" /* OperateResizeHandlers.LEFT_BOTTOM */ || command === "right-top" /* OperateResizeHandlers.RIGHT_TOP */)
                        revisedY = -revisedX / aspectRatio;
                }
                ...
                if (command === "right-bottom" /* OperateResizeHandlers.RIGHT_BOTTOM */) {
                    width = getSizeWithinRange(elOriginWidth + revisedX);
                    height = getSizeWithinRange(elOriginHeight + revisedY);
                }
                else if (command === "left-bottom" /* OperateResizeHandlers.LEFT_BOTTOM */) {
                    width = getSizeWithinRange(elOriginWidth - revisedX);
                    height = getSizeWithinRange(elOriginHeight + revisedY);
                    left = elOriginLeft - (width - elOriginWidth);
                }
                else if (command === "left-top" /* OperateResizeHandlers.LEFT_TOP */) {
                    width = getSizeWithinRange(elOriginWidth - revisedX);
                    height = getSizeWithinRange(elOriginHeight - revisedY);
                    left = elOriginLeft - (width - elOriginWidth);
                    top = elOriginTop - (height - elOriginHeight);
                }
                else if (command === "right-top" /* OperateResizeHandlers.RIGHT_TOP */) {
                    width = getSizeWithinRange(elOriginWidth + revisedX);
                    height = getSizeWithinRange(elOriginHeight - revisedY);
                    top = elOriginTop - (height - elOriginHeight);
                }
                else if (command === "top" /* OperateResizeHandlers.TOP */) {
                    height = getSizeWithinRange(elOriginHeight - revisedY);
                    top = elOriginTop - (height - elOriginHeight);
                }
                else if (command === "bottom" /* OperateResizeHandlers.BOTTOM */) {
                    height = getSizeWithinRange(elOriginHeight + revisedY);
                }
                else if (command === "left" /* OperateResizeHandlers.LEFT */) {
                    width = getSizeWithinRange(elOriginWidth - revisedX);
                    left = elOriginLeft - (width - elOriginWidth);
                }
                else if (command === "right" /* OperateResizeHandlers.RIGHT */) {
                    width = getSizeWithinRange(elOriginWidth + revisedX);
                }
            ...
            elementList.value = elementList.value.map(el => {
                ...
                if (el.type === 'shape' && 'pathFormula' in el && el.pathFormula) {
                    ...
                    if ('editable' in pathFormula)
                        path = pathFormula.formula(width, height, el.keypoint);
                    else
                ...
                if (el.type === 'table') {
                    ...
                    if (cellMinHeight === originTableCellMinHeight)
    ...
    // 多选元素缩放
    const scaleMultiElement = (e, range, command) => {
        let isMouseDown = true;
        ...
        const originElementList = JSON.parse(JSON.stringify(elementList.value));
        ...
        document.onmouseup = e => {
            ...
            document.onmousemove = null;
            document.onmouseup = null;
            if (startPageX === e.pageX && startPageY === e.pageY)
                return;
            slidesStore.updateSlide({ elements: elementList.value });
            addHistorySnapshot();
        };
