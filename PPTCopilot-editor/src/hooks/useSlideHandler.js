...
export default () => {
    ...
    const updateSlideIndex = (command) => {
        if (command === "ARROWUP" /* KEYS.UP */ && slideIndex.value > 0) {
            if (activeElementIdList.value.length)
                mainStore.setActiveElementIdList([]);
            slidesStore.updateSlideIndex(slideIndex.value - 1);
        }
        else if (command === "ARROWDOWN" /* KEYS.DOWN */ && slideIndex.value < slides.value.length - 1) {
            if (activeElementIdList.value.length)
                mainStore.setActiveElementIdList([]);
            slidesStore.updateSlideIndex(slideIndex.value + 1);
        }
...
//# sourceMappingURL=useSlideHandler.js.map