...
export default () => {
    ...
    const uniformHorizontalDisplay = () => {
        ...
        const copyOfActiveElementList = JSON.parse(JSON.stringify(activeElementList.value));
        const newElementList = JSON.parse(JSON.stringify(currentSlide.value.elements));
        // 分别获取普通元素和组合元素集合，并记录下每一项的范围
        const singleElemetList = [];
        let groupList = [];
        ...
        const list = [...singleElemetList, ...formatedGroupList];
        ...
        let lastPos = { min: firstItem.min, max: firstItem.max };
        if ('el' in firstItem) {
...
//# sourceMappingURL=useUniformDisplayElement.js.map