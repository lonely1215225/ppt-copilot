...
export default () => {
    ...
    const canCombine = computed(() => {
        if (activeElementList.value.length < 2)
            return false;
    ...
    const combineElements = () => {
        if (!activeElementList.value.length)
            return;
        // 生成一个新元素列表进行后续操作
        let newElementList = JSON.parse(JSON.stringify(currentSlide.value.elements));
        // 生成分组ID
    ...
    const uncombineElements = () => {
        ...
        if (!hasElementInGroup)
            return;
        const newElementList = JSON.parse(JSON.stringify(currentSlide.value.elements));
        for (const element of newElementList) {
            if (activeElementIdList.value.includes(element.id) && element.groupId)
...
//# sourceMappingURL=useCombineElement.js.map