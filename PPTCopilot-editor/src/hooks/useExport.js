...
import { toAST } from '@/utils/htmlParser';
...
export default () => {
    ...
    // 导出图片
    const exportImage = (domRef, format, quality, ignoreWebfont = true) => {
    ...
    const exportSpecificFile = (_slides) => {
    ...
    const formatPoints = (points, scale = { x: 1, y: 1 }) => {
            ...
            if (point.close !== undefined) {
                ...
                if (point.curve.type === 'cubic') {
                    return {
                        ...
                        curve: {
                            type: 'cubic',
                            x1: point.curve.x1 / INCH_PX_RATIO * scale.x,
                            y1: point.curve.y1 / INCH_PX_RATIO * scale.y,
                            x2: point.curve.x2 / INCH_PX_RATIO * scale.x,
                            y2: point.curve.y2 / INCH_PX_RATIO * scale.y,
                        },
                    };
                }
    ...
    // 获取边框配置
    const getOutlineOption = (outline) => {
    ...
    // 导出PPTX文件
    const exportPPTX = (_slides, masterOverwrite) => {
        exporting.value = true;
        const pptx = new pptxgen();
        if (viewportRatio.value === 0.625)
            pptx.layout = 'LAYOUT_16x10';
        else if (viewportRatio.value === 0.75)
        ...
        for (const slide of _slides) {
            ...
            for (const el of slide.elements) {
                if (el.type === 'text') {
                    ...
                    if (el.wordSpace)
                        options.charSpacing = el.wordSpace * PT_PX_RATIO;
                    if (el.lineHeight)
                    ...
                    if (el.themeColor.length === 10)
                        ...
                        chartColors = tinycolor(el.themeColor[0]).analogous(10).map(color => formatColor(color.toHexString()).color);
                    else {
