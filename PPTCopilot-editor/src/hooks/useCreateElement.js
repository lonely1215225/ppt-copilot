...
export default () => {
     ...
     * @param col 列数
     */
    const createTableElement = (row, col) => {
        const style = {
        ...
        const colWidths = new Array(col).fill(1 / col);
    ...
    const createTextElement = (position, data) => {
        ...
        createElement({
            ...
            setTimeout(() => {
                const editorRef = document.querySelector(`#editable-element-${id} .ProseMirror`);
                if (editorRef)
                    editorRef.focus();
            }, 0);
        });
    ...
    const createLineElement = (position, data) => {
        ...
        if (data.isCubic)
            newElement.cubic = [[(start[0] + end[0]) / 2, (start[1] + end[1]) / 2], [(start[0] + end[0]) / 2, (start[1] + end[1]) / 2]];
        createElement(newElement);
    };
    /**
     * 创建LaTeX元素
