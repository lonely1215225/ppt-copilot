...
export default () => {
    ...
    // 创建原颜色与新颜色的对应关系表
    const createSlideThemeColorMap = (slide, newColors) => {
    ...
    // 设置幻灯片主题
    const setSlideTheme = (slide, theme) => {
        ...
        for (const el of slide.elements) {
            ...
            if (el.type === 'table') {
                if (el.theme)
                    el.theme.color = colorMap[tinycolor(el.theme.color).toRgbString()] || el.theme.color;
    ...
    // 应用预置主题（单页）
    const applyPresetThemeToSingleSlide = (theme) => {
        const newSlide = JSON.parse(JSON.stringify(currentSlide.value));
    ...
    // 应用预置主题（全部）
    const applyPresetThemeToAllSlides = (theme) => {
        const newSlides = JSON.parse(JSON.stringify(slides.value));
...
//# sourceMappingURL=useSlideTheme.js.map