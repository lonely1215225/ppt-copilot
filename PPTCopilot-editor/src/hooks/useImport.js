...
import { SHAPE_LIST, SHAPE_PATH_FORMULAS } from '@/configs/shapes';
...
export default () => {
    ...
    // 导入pptist文件
    const importSpecificFile = (files, cover = false) => {
        const file = files[0];
        const reader = new FileReader();
        reader.addEventListener('load', () => {
            try {
                const slides = JSON.parse(decrypt(reader.result));
                if (cover)
...
//# sourceMappingURL=useImport.js.map